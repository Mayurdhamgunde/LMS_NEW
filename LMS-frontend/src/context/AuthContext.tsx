...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const login = async (email: string, password: string, loginTenantId?: string) => {
    try {
      ...
      setError(null)
      navigate('/ ')
      // const redirectPath = res.data.user.role=== 'student' ? '/learner' : '/'
      ...
      let userFriendlyError = 'Login failed. Please check your connection and try again.';
      
      if (errorMessage) {
        // Map backend error messages to user-friendly messages
        switch (errorMessage) {
          case 'User not found. Please check your email or sign up for a new account.':
            userFriendlyError = 'User not found. Please check your email or sign up for a new account.';
            break;
          case 'Incorrect password. Please try again.':
            userFriendlyError = 'Incorrect password. Please try again.';
            break;
          case 'Tenant ID is required':
            userFriendlyError = 'Organization is required. Please select a valid organization.';
            break;
  ...
  const register = async (name: string, email: string, password: string, registerTenantId?: string, role?: string) => {
    try {
      ...
      // Handle specific error messages from backend
      const errorMessage = err.response?.data?.error;
      let userFriendlyError = 'Registration failed. Please check your connection and try again.';
      
      if (errorMessage) {
        // Map backend error messages to user-friendly messages
        switch (errorMessage) {
          case 'Email is already registered. Please use a different email or try logging in.':
