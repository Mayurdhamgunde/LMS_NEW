...
const Layout = ({ toggleDarkMode, darkMode }: LayoutProps) => {
  ...
  const themeClasses = {
    ...
    navbarBg: darkMode ? 'bg-[#1e2736]' : 'bg-white',
  ...
  const isStudent = ['student', 'learner'].includes(user?.role?.toLowerCase() || '')
  ...
  const navigationItems = [
    ...
    //   label: isStudent ? 'Knowledge Check' : 'Assignments',
    //   roles: ['admin', 'instructor']
    // },
    // Admin and Instructor only items
    {
      path: '/admin',
  ...
  const filteredNavigationItems = navigationItems.filter(item => 
    item.roles.some(role => user?.role?.toLowerCase() === role.toLowerCase())
  )

  // For students, we'll use a horizontal navbar with navigation on the right
  const renderStudentLayout = () => (
    <div className={`min-h-screen ${themeClasses.bg}`}>
      {/* Top navigation for student */}
      <header className={`sticky top-0 z-20 ${themeClasses.navbarBg} border-b ${themeClasses.border}`}>
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <Link to="/" className="flex items-center gap-2">
            ...
            <span className={`${themeClasses.text} font-bold tracking-wide`}>
    {user?.tenantId === 'default' ? 'Learnomic' : user?.tenantId === 'ngo' ? 'NobleGiving' : 'LMS System'}
  </span>
          </Link>
          
          {/* Navigation items on the right */}
          <div className="flex items-center gap-4">
            {/* Horizontal navigation for student - moved to right */}
            <nav className="hidden md:flex items-center gap-1">
              {filteredNavigationItems.map((item, index) => (
                <button
                  key={index}
                  onClick={() => navigate(item.path)}
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all cursor-pointer ${getActiveLinkClass(item.path)}`}
                >
                  {item.icon}
        ...
        {drawerOpen && (
          <div className={`md:hidden ${themeClasses.navbarBg} border-t ${themeClasses.border}`}>
            <nav className="p-2 grid grid-cols-2 gap-1">
              {filteredNavigationItems.map((item, index) => (
                <button
                  key={index}
                  onClick={() => { navigate(item.path); setDrawerOpen(false); }}
                  className={`flex flex-col items-center gap-1 px-3 py-2 rounded-lg transition-all cursor-pointer ${getActiveLinkClass(item.path)}`}
                >
                  {item.icon}
      ...
      {/* Page content */}
      <main className={`min-h-[calc(100vh-64px)] ${themeClasses.bg}`}>
        <Outlet />
      </main>
    </div>
  )

  // For admin/instructor, use the original sidebar layout
  const renderAdminInstructorLayout = () => (
    <>
      <div className={`min-h-screen ${themeClasses.bg}`}>
        {/* Mobile backdrop */}
        {drawerOpen && (
        ...
        {/* Sidebar - Mobile */}
        <div 
          className={`fixed inset-y-0 left-0 w-64 ${themeClasses.navbarBg} transform transition-transform duration-300 ease-in-out z-40 lg:hidden ${
          ...
          <nav className="p-4 space-y-2">
            {filteredNavigationItems.map((item, index) => (
              <button 
                ...
                className={`flex items-center w-full gap-3 px-3 py-2 rounded-lg transition-all cursor-pointer ${getActiveLinkClass(item.path)}`}
              >
                {item.icon}
                <span>{item.label}</span>
              </button>
        ...
        {/* Sidebar - Desktop */}
        <div className={`hidden lg:fixed lg:inset-y-0 lg:flex lg:flex-col lg:w-64 ${themeClasses.navbarBg} border-r ${themeClasses.border}`}>
          <div className={`flex items-center gap-3 px-4 py-3 border-b ${themeClasses.border}`}>
            <div className="p-2 bg-gradient-to-r from-blue-500 to-purple-500 rounded-lg transition-transform hover:scale-105">
              <AcademicCapIcon className="w-5 h-5 text-white" />
            </div>
  ...
  return (
    <>
      {isStudent ? renderStudentLayout() : renderAdminInstructorLayout()}
